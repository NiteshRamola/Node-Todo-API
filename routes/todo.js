...
router.put('/:id', async (req, res) => {
  ...
  if (!todo) return res.status(404).send('The task with the given ID was not found.');
  if(data._id !== todo.user.toString()) return res.status(403).send('You are not authorized to update this task.');
...
router.patch('/:id', async (req, res) => {
  ...
  if (!todo) return res.status(404).send('The task with the given ID was not found.');
  if(data._id !== todo.user.toString()) return res.status(403).send('You are not authorized.');
